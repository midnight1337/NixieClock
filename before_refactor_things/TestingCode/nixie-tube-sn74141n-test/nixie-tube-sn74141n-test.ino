...
*/
// weź kurwa zobacz ten filmik co sie nazywał jakoś, one button multi activities czy jakoś tak
#include <DS3231.h>
#include <Wire.h>
...
// Arduino onboard defined SN74141N_x pinout
...
// Defined SN74141N input logic states
...
void CheckForMicroswitchPress()
{
  ...
  int pressedTime = 0;
  // if switch_A3 is pressed then enter if statement
  if (analogRead(switch_A3) == LOW)
  {
    ...
    while (true)
    {
      ...
      // TUTAJ DOROBIC ZE JAK KROTKO WCISNIETE TO PRZESKAKUJE A JAK DLUGO TO ZAPISUJE. (FILMIK ONE BUTTON MULTIPLE FUNCTIONS??)
      // if switch_A3 is pressed, then change on next one, or set current time and exit function
      if (analogRead(switch_A3) == LOW)
      {
